            ...
            <div class="col-md-12">
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
                        ...
                        <wo-table
                            :total-records="getDistricts ? getDistricts.length : 0"
                            :remote="false"
                            :columns="columns"
...
<script>
    ...
    import DistrictModal from '../../Modals/DistrictModal.vue';

    import { createNamespacedHelpers } from 'vuex';
    const { mapActions, mapGetters } = createNamespacedHelpers('administration');

    export default {
            components: { WoTable, DistrictModal },
        ...
        methods: {
        ...mapActions(['allDistrictAction', 'deleteDistrictAction', 'updateDistrictAction']),

            deleteItem(row) {
                helper.deletingAlert("district will be deleted")
                .then((result) => {
                    if (result.value) {
                        this.isLoading = true;
                        this.deleteDistrictAction(row)
                        .then((res) => {
                            this.isLoading = false;
                            helper.successAlert("district deleted");
                        }).catch((error) => {
                            this.isLoading = false;
