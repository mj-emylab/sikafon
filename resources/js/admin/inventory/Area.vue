            ...
            <div class="col-md-12">
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
                        ...
                        <wo-table
                            :total-records="getAreas ? getAreas.length : 0"
                            :remote="false"
                            :columns="columns"
...
<script>
    ...
    export default {
        ...
        methods: {
        ...mapActions(['allAreaAction', 'deleteAreaAction', 'updateAreaAction']),

            deleteItem(row) {
                helper.deletingAlert("area will be deleted")
                .then((result) => {
                    if (result.value) {
                        this.isLoading = true;
                        this.deleteAreaAction(row)
                        .then((res) => {
                            this.isLoading = false;
                            helper.successAlert("area deleted");
                        }).catch((error) => {
                            this.isLoading = false;
            ...
            showModal() {
                this.$refs.area.openAddModal();
            },

            editModal(row) {
                this.$refs.area.openEditModal(row);
            },
        },
        created() {
            this.allAreaAction();
