<template>
    <div>
        <modal ref="woModal" modal-class-name="AddRegionalManager" :modalClasses="'modal-md'">
            <template slot="header">
                <div class="modal-title row">
                    <h2 class="col-12">
                        <span><b>{{ addOrUpdate ? 'Edit' : 'Add' }}</b></span>
                    </h2>
                </div>
            </template>
            <template slot="close-button">
                <button @click.prevent="hideModal" title="Click to close modal" class="btn btn-danger btn-sm">
                    <!-- <i class="fa fa-window-close"></i> -->
                    <span>&times;</span>
                </button>
            </template>
            <template>

                <form @submit.prevent="saveData" id="myForm">

                    <loading
                        :active="saveLoading"
                        :is-full-page="false"
                        loader="bars"
                        :width="20"
                        :height="20"/>

                    <div class="row mt-3 mb-3">
                        <div class="col-md-12 mb-3">
                            <label><b>Region</b></label>
                            <v-select
                                :options="getRegions"
                                :label="'name'"
                                :reduce="getRegions => getRegions.id"
                                v-model="formData.region_id"
                                placeholder="Select Region">

                                <template #search="{attributes, events}">
                                    <input
                                        class="vs__search"
                                        :required="!formData.region_id"
                                        v-bind="attributes"
                                        v-on="events" />
                                </template>
                            </v-select>
                        </div>
                        <div>
                            <custom-input
                                :label="'Findme ID'"
                                v-model="formData.findmeId"
                                :required="true"
                                type="text"
                                placeholder="Enter Findme ID"/>
                        </div>

                    </div>

                </form>
            </template>
            <template slot="footer">
                
                <button @click.prevent="hideModal" title="Click to close modal" class="btn btn-danger btn-sm">
                    <i class="fa fa-window-close"></i>
                    <span>Close</span>
                </button>
                <button form="myForm" type="submit" title="Click to save data" class="btn btn-primary">
                    <i class="fa fa-save"></i>
                    <span>Save</span>
                </button>
            </template>
         </modal>
    </div>
</template>
<script>
import Modal from "../components/CustomModal.vue";
import helper from '../helpers/helpers';
import { createNamespacedHelpers } from 'vuex';
import CustomInput from '../components/CustomInput.vue';
const { mapActions, mapGetters } = createNamespacedHelpers('administration');

export default {
    components: {
        Modal,
        CustomInput,
    },
    data() {
        return {
            saveLoading:false,
            addOrUpdate: false,


            formData: {
                id: 0,
                findmeId: "",
                region_id: 0,

            },
        }
    },
    methods: {
        ...mapActions(['updateRegionalManagerAction', 'saveRegionalManagerAction', 'allRegionalManagerAction', 'allRegionAction']),

        openAddModal() {
            this.clearData();
            this.addOrUpdate = false;
            helper.openModal(".AddRegionalManager");
            // $(".AddRegionalManager").modal('show');
        },

        openEditModal(row) {
            this.clearData();
            this.formData.id= row.id;
            this.formData.findmeId= row.findme_id;
            this.formData.region_id= row.region_id,



            this.addOrUpdate = true;
            helper.openModal(".AddRegionalManager");
            // $(".AddRegionalManager").modal('show');
        },

        hideModal() {
            helper.hideModal(".AddRegionalManager");
            // $(".AddRegionalManager").modal('hide');
        },

        clearData(){
            this.formData.id = "";
            this.formData.findmeId = "";
            this.formData.region_id = 0;
        },

        saveData() {
            // this.saveLoading = true;
            if(this.addOrUpdate) {
                const data = this.formData;
                this.updateRegionalManagerAction(data).then((res) => {
                    this.saveLoading = false;
                    helper.successAlert("Updated Successfully");
                    this.clearData()
                    this.hideModal();
                }).catch((error) => {
                    this.saveLoading = false;
                    helper.errorMessage(error);
                });
            } else {
                const data = this.formData;
                this.saveRegionalManagerAction(data).then((res) => {
                    this.saveLoading = false;
                    this.allRegionalManagerAction();
                    this.hideModal();
                    helper.successAlert("Created Successfully");
                    this.clearData()
                }).catch((error) => {
                    this.saveLoading = false;
                    helper.errorMessage(error);
                });
            }

        },

    },
    created() {
        this.allRegionAction()
    },
    computed: {
        ...mapGetters(['getRegions']),
    },
    mounted() {

    },

    beforeDestroy() {
        
    },
    destroyed() {
        
    },
}
</script>
<style scoped>

</style>
