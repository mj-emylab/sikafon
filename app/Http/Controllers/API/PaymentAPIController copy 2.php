...
class PaymentAPIController extends AppBaseController
{
    ...
    public function store(CreatePaymentAPIRequest $request)
    {
        ...
        try {
            ...
            switch ($request->trans_type) {
                case '1': // withdraw
                    $fromAccountUser = AccountUser::where('user_id', $code->user_id)->first();
                    ...
                    if (empty($fromAccountUser)) {
                        return $this->sendSuccess('Agent not found');
                    }

                    $sendername = $agent->name;
                    $subagentcode = $agent->agent_code; // "90808987"
                    $senderphone = $agent->phone; // "23324412345678"
    ...
    public function account(Request $request)
    {
        ...
        try {
            $accountRefNo = $account->account_no; // "CUPD0000000120982"
    ...
    {
        /** @var Payment $payment */
        $account = UserAccount::where('account_no', $request->account_no)->first();
