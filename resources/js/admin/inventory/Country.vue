            ...
            <div class="col-md-12">
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
                        ...
                        <wo-table
                            :total-records="getCountries ? getCountries.length : 0"
                            :remote="false"
                            :columns="columns"
...
<script>
    ...
    import CountryModal from '../../Modals/CountryModal.vue';

    import { createNamespacedHelpers } from 'vuex';
    const { mapActions, mapGetters } = createNamespacedHelpers('administration');

    export default {
            components: { WoTable, CountryModal },
        ...
        methods: {
        ...mapActions(['allCountryAction', 'deleteCountryAction', 'updateCountryAction']),

            deleteItem(row) {
                helper.deletingAlert("country will be deleted")
                .then((result) => {
                    if (result.value) {
                        this.isLoading = true;
                        this.deleteCountryAction(row)
                        .then((res) => {
                            this.isLoading = false;
                            helper.successAlert("country deleted");
                        }).catch((error) => {
                            this.isLoading = false;
