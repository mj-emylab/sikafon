            ...
            <div class="col-md-12">
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
                        ...
                        <wo-table
                            :total-records="getRegions ? getRegions.length : 0"
                            :remote="false"
                            :columns="columns"
...
<script>
    ...
    import RegionModal from '../../Modals/RegionModal.vue';

    import { createNamespacedHelpers } from 'vuex';
    const { mapActions, mapGetters } = createNamespacedHelpers('administration');

    export default {
            components: { WoTable, RegionModal },
        ...
        methods: {
        ...mapActions(['allRegionAction', 'deleteRegionAction', 'updateRegionAction']),

            deleteItem(row) {
                helper.deletingAlert("region will be deleted")
                .then((result) => {
                    if (result.value) {
                        this.isLoading = true;
                        this.deleteRegionAction(row)
                        .then((res) => {
                            this.isLoading = false;
                            helper.successAlert("region deleted");
                        }).catch((error) => {
                            this.isLoading = false;
