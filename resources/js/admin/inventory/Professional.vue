<template>
  ...
  <div>
            ...
            <div class="col-md-12">
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
                        ...
                        <wo-table
                            :total-records="getProfessionals ? getProfessionals.length : 0"
                            :remote="false"
                            :columns="columns"
                            :enable-check-box="false"
                            :rows="getProfessionals">
                            <template #view="{row}">
                                <router-link class="badge" style="text-color: white; background-color:#2E8B57" :to="{path:'/view-professional',query:{id: row.id}}" >View</router-link>
                            </template>
                            <template #status="{row}">
    ...
    <professional-modal ref="professional" />
...
<script>
    ...
    import ProfessionalModal from '../../Modals/ProfessionalModal.vue';
    ...
    export default {
            components: { WoTable, ProfessionalModal, GalleryModal, SessionModal, ViewModal },

            data() {
                return {
                isLoading: false,
                columns: [
                    { label: 'Name', field: 'pro_title'},
                    { label: 'Findme ID', field: 'professional_code'},
        ...
        methods: {
        ...mapActions(['allProfessionalAction', 'deleteProfessionalAction', 'updateProfessionalAction']),

            deleteItem(row) {
                helper.deletingAlert("professional will be deleted")
                .then((result) => {
                    if (result.value) {
                        this.isLoading = true;
                        this.deleteProfessionalAction(row)
                        .then((res) => {
                            this.isLoading = false;
                            helper.successAlert("professional deleted");
                        }).catch((error) => {
                            this.isLoading = false;
